                ...
                <form method="post">
                  ...
                  <div class="form-group">
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST") { // check if the request is a POST request
                      $service_type = $_POST['service_type']; // populate variable with the form value of service type
                      $comment = $_POST['comment']; // populate variable with the comment data from the form
                      $date = $_POST['date']; // populate the variable with the date data from the form
                      $cus_email = $_SESSION["login_gergarage"]; // populate the variable with the session email data
                      $vehicle_name = $_POST['vehicle_license']; // populate the variable with the form vehicle license data
                      // echo '<script type="text/javascript">alert("'.$vehicle_name.'");</script>';//working

                      // Count bookings from this date in list of bookings
                      $sql = "SELECT COUNT(date) FROM booking 
                      where date = '$date';";

                      $result = mysqli_query($conn, $sql); // kick off sql query
                      $count1 = mysqli_fetch_row($result); // create count variable by counting number of booking results
                      $count = implode(" ", $count1); // break apart array count1


                      if ($count < 16) { // check if there is less than 16 results (not sure why 16)
                        //Insert into booking =====================================================  e essa mano
                        $sql2 = "SELECT license FROM vehicle 
                        where customer_email = '$cus_email' and type = '$vehicle_name';"; //retrieve license value from vehicle table where customer email equals logged in email and type equals user selected vehicle

                        $result1 = mysqli_query($conn, $sql2); // kick off sql query
                        $vehicle_license1 = mysqli_fetch_row($result1); //array
                        $vehicle_license11 = implode(" ", $vehicle_license1); // break apart array 


                        //Insert into BOOKING Table
                        $sql3 = "INSERT INTO `booking` ( `service_type`, `comment`, `date`, `customer_email`, `vehicle_license`, `status`, `staff_id_staff`) 
                        VALUES('$service_type', '$comment', '$date','$cus_email', '$vehicle_license11', 'Booked', 1);"; // Insert booking information into booking table
                        $result = mysqli_query($conn, $sql3); // kick off sql query above

                        //Insert into BOOKING Table where date is equal to selected date and vehicle licence equals the retrieved vehicle license value from above
                        $sql4 = "SELECT * from booking  
                        where date = '$date'
                        and vehicle_license = '$vehicle_license11';";

                        $result2 = mysqli_query($conn, $sql4); // kick off sql query above
                        $row = mysqli_fetch_assoc($result2); // array
                        $id_booking = $row['id_booking']; // populate variable with retrieved ID value

                        //Insert into Invoice Table with retrieved booking id
                        ...
                        if ($service_type === "Major Repair") {
                          //Update into Invoice Table, set price to 160, where id is equal to retrieved ID
                          $sql34 = "UPDATE invoice 
                                    SET total_price= 160
                                    WHERE booking_id_booking = $id_booking;";
                          $result2 = mysqli_query($conn, $sql34);

                          //save value to Services table, price and retrieved ID
                        ...
                        if ($service_type === "Annual Service") {
                          //Update into Invoice Table, set price to 235, where booking id matches retrieved ID
                          $sql34 = "UPDATE invoice 
                          SET total_price= 235
                          WHERE booking_id_booking = $id_booking;";
                        ...
                        echo '<script>alert("There is no available time")</script>'; //create alert saying there isn't a free slot
                      }
                    }
                    ?>
